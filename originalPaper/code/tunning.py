...
def hisgram(lis, n_bin=100, his_norm_flag='yes', lowerbound=-1, upperbound=1, cdf_flag=False, uniform_flag=True):
    ...
    if cdf_flag == True:
        ...
        ecdf = ECDF(lis)
        if uniform_flag:
            return ecdf([i / float(n_bin) for i in range(0, n_bin)])
        else:
            return ecdf([i / float(n_bin) for i in range(0, n_bin)])
...
def get_all_feature_keys(node_descriptors, aggregators):
    keys = []
    for descr in node_descriptors:
        keys.append(descr)
        keys.extend([f'1_0_{descr}_{agg}' for agg in aggregators])
    return keys

def merge_features(graphs_, feature_keys, n_bin=30, his_norm_flag='yes', edge_flag=False, cdf_flag=False, uniform_flag = True):
